...
public class AreaPersonaleControl extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (action != null && action.equalsIgnoreCase("ShowGioco")) {
			...
			try {
				...
				if (isAjaxRequest) {
					...
					RequestDispatcher dispatcher = getServletContext()
							.getRequestDispatcher("/fragments/showAccessorio.jsp");
					dispatcher.include(request, response);
				} else {
					request.removeAttribute("prodottiAP");
					request.setAttribute("prodottiAP", accessori);
			...
			try {
				...
				if (isAjaxRequest) {
					response.setContentType("text/html");
					RequestDispatcher dispatcher = getServletContext()
							.getRequestDispatcher("/fragments/showCards2.jsp");
					dispatcher.include(request, response);
				} else {
					request.removeAttribute("prodottiAP");
					request.setAttribute("prodottiAP", carte);
			...
			try {
				...
				// Ottieni un dispatcher per il JSP dei dettagli della carta (o crea HTML
				// direttamente)
				RequestDispatcher dispatcher = getServletContext()
						.getRequestDispatcher("/fragments/showCardDetails.jsp");
				dispatcher.include(request, response);
			} catch (SQLException e) {
			...
			try {
				...
				if (isAjaxRequest) {
					response.setContentType("text/html");
					RequestDispatcher dispatcher = getServletContext()
							.getRequestDispatcher("/fragments/showEspansione.jsp");
					dispatcher.include(request, response);
				} else {
					request.removeAttribute("prodottiAP");
					request.setAttribute("prodottiAP", espansioni);
		...
		if (action != null && action.equals("recensione")) {
			...
			if (codiceOrdineStr != null && username != null) {
				...
				try {
					...
					if (recensione != null) {
						// Se la recensione è trovata, setta l'attributo nella request e reindirizza a
						// showReview.jsp
						request.setAttribute("recensione", recensione);
						request.setAttribute("insVis", 1);
						RequestDispatcher dispatcher = getServletContext()
								.getRequestDispatcher("/fragments/showReview.jsp");
						dispatcher.forward(request, response);
					} else {
		...
		if (action != null && action.equals("insertRev")) {
			...
			// Dopo l'inserimento, reindirizza a showReview.jsp per visualizzare le
	...
	protected String saveImmagine(HttpServletRequest request, HttpServletResponse response, String type, int ch)
			throws ServletException, IOException {
		...
		String savePath = applicationPath + File.separator + "IMMAGINI" + File.separator + "IMMAGINI_"
				+ type.toUpperCase();
